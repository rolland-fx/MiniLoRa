...
#define BLE false
#define ZIGBEE false
#define LORAWAN false
#define LOCAL_STORE false

#define GNSS true
#define SHT40 false // all SHT40 is present for previous proto. Will be removed ASAP
#define BME280 true
#define OPT30 true
#define LIS3DH true // for now, cause issue with the SPI for LoRa. Will be fixed later. Due to the library used

const gpio_num_t LORA_RST = GPIO_NUM_2;
const gpio_num_t GNSS_RST = GPIO_NUM_0;
const gpio_num_t LED_ORANGE = GPIO_NUM_15;
const gpio_num_t BUTTON = GPIO_NUM_7;
const gpio_num_t BAT_ON = GPIO_NUM_4;
const gpio_num_t BAT_LV = GPIO_NUM_5;
const gpio_num_t PPS = GPIO_NUM_6;
const gpio_num_t EXT_GPIO = GPIO_NUM_4;
const gpio_num_t INT_ACC = GPIO_NUM_1;

struct Data { //Total size : 57 bytes. To Store data with all the precisions
  int8_t Temp;
  uint8_t Hum;
  uint16_t Pres;
  uint16_t Bat_vol;
...
struct LoRaWanData { //Total size : 25 bytes. To send via LoRaWan
  int8_t Temp;
  uint8_t Hum;
  uint8_t Pres;
  uint8_t Bat_vol;
...
void setup() {
  ...
  if (BME280){
    ...
    LiveData.Temp = (int8_t)BMEdata->temperature;
    LiveData.Hum = (uint8_t)BMEdata->humidity;
    LiveData.Pres = (uint16_t)(BMEdata->pressure / 100);
  }
  if (OPT30) {
  ...
  if (LORAWAN) {
    ...
    LoRaData.Pres = (uint8_t)(LiveData.Pres - 800); 
